const App = {
    ...
    switchView: (view) => {
        ...
        document.querySelectorAll('[id^="btn-view-"]').forEach(el => el.classList.remove('bg-vibrantCyan/20', 'text-vibrantCyan', 'border-vibrantCyan/50'));

        document.getElementById('view-' + view).classList.remove('hidden');
        document.getElementById('btn-view-' + view).classList.add('bg-vibrantCyan/20', 'text-vibrantCyan', 'border-vibrantCyan/50');
    ...
    renderFileList: () => {
        ...
        list.innerHTML = App.pendingFiles.map((f, i) => `
            <div class="file-item flex justify-between items-center p-2 bg-slate-800 border border-slate-700 rounded text-xs">
                <span class="font-mono text-slate-300 truncate">${f.name}</span>
    ...
    renderDashboard: () => {
        ...
        kpiSaldo.className = `text-2xl font-bold ${saldo >= 0 ? 'text-vibrantCyan' : 'text-red-500'}`;
        ...
        App.charts.pie = new Chart(document.getElementById('chart-pie'), {
            ...
            data: {
                ...
                datasets: [{ data: topNat.map(x=>x[1]), backgroundColor: ['#00B4D8','#ef4444','#f59e0b','#10b981','#3b82f6','#6366f1'] }]
        ...
        const fillTable = (map, id) => {
            ...
            document.getElementById(id).innerHTML = sorted.map(i => `
                <tr class="border-b border-slate-800"><td class="p-3 truncate max-w-[150px] text-slate-300" title="${i.cc}">${i.cc}</td><td class="p-3 text-slate-500 truncate max-w-[150px]" title="${i.n}">${i.n}</td><td class="p-3 text-right font-bold text-slate-200">${fmt.format(i.v)}</td></tr>
            `).join('');
        }
    ...
    renderDebugTable: () => {
        ...
        tbody.innerHTML = mixed.map(row => `
            <tr class="hover:bg-slate-800/50 transition border-b border-slate-800">
                <td class="p-3 border-r border-slate-800 font-mono text-xs text-slate-500">${row.date.toLocaleDateString()}</td>
                <td class="p-3 border-r border-slate-800 font-bold ${row.type==='RECEITA'?'text-trustGreen':'text-red-500'} text-[10px] uppercase">${row.type}</td>
                <td class="p-3 border-r border-slate-800 font-medium text-slate-300 truncate max-w-[200px]" title="${row.nature}">${row.nature}</td>
                <td class="p-3 border-r border-slate-800 text-slate-400 truncate max-w-[200px]" title="${row.cc}">${row.cc}</td>
                <td class="p-3 text-right font-bold font-mono text-slate-200">${fmt.format(row.val)}</td>
            </tr>
        `).join('');
    },
    ...
    populateReportFilters: () => {
        ...
        document.getElementById('report-filter-list').innerHTML = nats.map(n => `
            <label class="flex items-center gap-2 text-slate-300"><input type="checkbox" value="${n}" class="report-check accent-vibrantCyan" checked><span>${n}</span></label>
        `).join('');
    },
    ...
    generateReportPreview: () => {
        ...
        document.getElementById('report-paper').innerHTML = `
            <div class="space-y-8 p-8 bg-white text-slate-900">
                <div class="flex justify-between border-b-2 border-slate-800 pb-4">
                    ...
                    <div class="text-right">
                        <div class="text-xl font-bold text-vibrantCyan">RDP Studio</div>
                ...
                <div class="mt-8 text-xs text-slate-400 text-center border-t pt-4">Gerado automaticamente pelo Sistema Financeiro Inteligente RDP</div>
            </div>
        `;
